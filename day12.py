...
def solution(rows, columns, queries):
    ...
    for query in queries:
        # 문제는 인덱스가 1부터 시작인데 배열은 0부터 시작이라 보정
        x1, y1, x2, y2 = query[0]-1, query[1]-1, query[2]-1, query[3]-1
        
        # 시작점의 값을 임시 백업하고, 이 값을 최솟값의 시작으로 설정
        temp = board[x1][y1]
        mini = temp
        
        # 왼쪽 변에서 아래에 있는 값을 위로 당기기
        for k in range(x1, x2):
            board[k][y1] = board[k+1][y1]
            mini = min(mini, board[k][y1])
            
        # 아래쪽 변에서 오른쪽에 있는 값을 왼쪽으로 당기기
        for k in range(y1, y2):
            board[x2][k] = board[x2][k+1]
            mini = min(mini, board[x2][k])
            
        # 오른쪽 변에서 위에 있는 값을 아래로 당기기
        for k in range(x2, x1, -1):
            board[k][y2] = board[k-1][y2]
            mini = min(mini, board[k][y2])
            
        # 위쪽 변에서 왼쪽에 있는 값을 오른쪽으로 당기기
        for k in range(y2, y1, -1):
            board[x1][k] = board[x1][k-1]
            mini = min(mini, board[x1][k])
